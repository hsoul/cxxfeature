...
const uint16_t __single_hash_x3[256] =
{
...
void xd3_compute_code_table_string (const xd3_dinst *code_table, uint8_t *str)
{
	...
	for (s = 0; s < 6; s += 1)
	{
		for (i = 0; i < 256; i += 1)
		{
			...
			{
			case 0:
				*str++ = (code_table[i].type1 >= XD3_CPY ? XD3_CPY : code_table[i].type1);
				break;
			case 1:
				*str++ = (code_table[i].type2 >= XD3_CPY ? XD3_CPY : code_table[i].type2);
				break;
			case 2:
			...
			case 3:
				*str++ = (code_table[i].size2);
				break;
			case 4:
				*str++ = (code_table[i].type1 >= XD3_CPY ? code_table[i].type1 - XD3_CPY : 0);
				break;
			case 5:
				*str++ = (code_table[i].type2 >= XD3_CPY ? code_table[i].type2 - XD3_CPY : 0);
				break;
			}
		}
	}
...
xd3_iopt_finish_encoding (xd3_stream *stream, xd3_rinst *inst)
{
	...
	if (stream->iout != NULL)
	{
		...
		{
			if ((ret = xd3_emit_double (stream, stream->iout, inst, stream->iout->code2)))
			{
				return ret;
			}
...
exit:
	if (ret != 0)
	{
		...
		//TODO: ²âÊÔÓÃ
		if (output && output_size_max > 128 && output_size)
		{
			strncpy((char *)output, stream.msg, 128);
			*output_size = strlen(stream.msg);
		}
	}
