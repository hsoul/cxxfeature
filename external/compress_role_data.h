#ifndef _COMP_ROLE_H_
#define _COMP_ROLE_H_
...
inline int DataDiff(const Octets& new_data, const Octets& old_data, Octets& output)
{
	...
	int ret = xd3_encode_memory((const uint8_t*)new_data.begin(), new_data.size(),
			(const uint8_t*)old_data.begin(), old_data.size(),
			obuf, &osize, MAX_DIFF_DATA_BUFF_SIZE, eflags);
...
inline int PatchDiff(const Octets& inputdata, const Octets& old_data, Octets& output)
{
	...
	int ret = xd3_decode_memory((const uint8_t*)inputdata.begin(), inputdata.size(),
			(const uint8_t*)old_data.begin(), old_data.size(),
			obuf, &osize, MAX_DIFF_DATA_BUFF_SIZE, 0);
...
public:
	...
	void OnChangeWorld(ruid_t roleid, const Octets& data)
	{
		...
		Log::log(LOG_DEBUG, "DS::Compress roleid:%ld, data:%zu, diff:%zu, ret:%d, ret2:%d, undiff_error:%d/a:%d/ad:%zu",
			roleid, data.size(), diff.size(), ret, ret2, undiff_error, (unsigned int)(data_size / data_count), (size_t)(diff_size / diff_count));

		if(undiff_error % 19 == 1 && data_count % 100 == 0)
		{
			Log::log(LOG_ERR, "DS::Compress undiff_error:%d, aver_data_sz:%d, count:%d, ave_diff_sz:%zu, count:%u.",
				undiff_error, (unsigned int)(data_size / data_count), data_count, (size_t)(diff_size / diff_count), diff_count);
		}

		//TEST_DATA_MODIFY_TIME 超过时间了，更换数据
